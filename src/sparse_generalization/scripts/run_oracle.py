...
def main(cfg: DictConfig):
    ...
    test_dataset_ind = EdgeDataset(
        data["X_test_ind"], data["Y_test_ind"], data["edges_test_ind"]
    )
    test_dataset_ood = EdgeDataset(
        data["X_test_ood"], data["Y_test_ood"], data["edges_test_ood"]
    )
    ...
    if cfg.seeds is None:
        ...
        test_loader_ind = DataLoader(
            test_dataset_ind, cfg.data.batch_size, shuffle=False, collate_fn=collate_fn
        )
        test_loader_ood = DataLoader(
            test_dataset_ood, cfg.data.batch_size, shuffle=False, collate_fn=collate_fn
        )
        ...
        for seed in cfg.seeds:
            ...
            if cfg.save:
                torch.save(
                    model.state_dict(), f"checkpoints/{cfg.run_name}_{timestamp}.pt"
                )
            ...
            table.add_data("Test set ID", test_metrics_1["loss"], test_metrics_1["acc"])
            table.add_data(
                "Test set OOD", test_metrics_2["loss"], test_metrics_2["acc"]
            )
            logger.experiment.log({"Test Sets Table": table})
            logger.experiment.finish()
