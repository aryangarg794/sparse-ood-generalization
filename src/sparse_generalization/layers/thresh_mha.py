...
class MultiHeadAttentionThresh(nn.Module):
    ...
    def _attention(
        self: Self, query: Tensor, key: Tensor, value: Tensor, return_mask: bool = False
    ):
        ...
        thresholded_probs = (
            thresholded_probs - attention_probs.detach() + attention_probs
        )  # ste

        hidden_repr = torch.bmm(thresholded_probs, value)  # (b*h, l, d)

        if return_mask:
            return hidden_repr.view(
                -1, self.heads, seq_len, self.dk
