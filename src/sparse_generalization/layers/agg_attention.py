...
class ExtraTokenAttention(nn.Module):
    ...
    def forward(self, x: Tensor, sum_heads: bool = True):
        ...
        keys = self.keys(x)  # (b, l+1, d)
        values = self.values(x)  # (b, l+1, d)

        queries_split = self._split_heads(queries)  # (b * h, l+1, d_k)
        keys_split = self._split_heads(keys)
        values_split = self._split_heads(values)

        if self.separate_mask:
            queries_mask = self.queries_mask(x)
            queries_mask_split = self._split_heads(queries_mask)
            keys_mask = self.keys_mask(x)
            keys_mask_split = self._split_heads(keys_mask)
    ...
    def _attention(
        ...
        return hidden_repr.view(-1, self.heads, seq_len, self.dk), attention_probs.view(
            -1, self.heads, seq_len, seq_len
        )
