...
from sparse_generalization.data.box_world.objects import (
    Wall,
    Goal,
    KeyBox,
    LockBox,
...
from sparse_generalization.data.box_world.constants import COLOR_NAMES, OOD_COLOR_NAMES

# somewhat inspired from https://github.com/aryangarg794/rnd_dqn_four_room/blob/master/four_room/env.py


class EnvBase(MiniGridEnv):
    def __init__(
        self,
        size=12,
        max_steps=100,
        ...
        mission = MissionSpace(mission_func=lambda: "reach the goal")
        self.size = size
    ...
    def _simple_wall_gen(self, min_size=2):
        walls = []
        orient = self.np_random.integers(0, 2)
        place = self.np_random.choice(
            list(range(2 + min_size, self.size - 2 - min_size))
        )  # at least 1 space at start, or 2 atr end
        door_size = self.np_random.integers(1, self.size - 3)
        door_place = self.np_random.integers(1, self.size - door_size)
    ...
    def add_distractors(
        ...
        for i in range(num_dist):
            ...
            pos = tuple(self.np_random.choice(valid_pos))
            rand_color = self.np_random.choice(colors)
...
class BoxWorldEnv(EnvBase):
    ...
    def __init__(
        ...
        self._randomize = (
            True
            if (
                self._agent_dir_list is None
                and self._goal_pos_list is None
                and self._agent_pos_list is None
                and self._topology_list is None
                and self._key_topology_list is None
            )
            else False
        )
        ...
        super().__init__(
            size=size, max_steps=max_steps, highlight=highlight, render_mode=render_mode
        )
    ...
    def _gen_grid(self, width, height):
        ...
        if self._randomize:
            if self.randomize_num_pairs:
                self.num_pairs = self.np_random.integers(1, 5)
            ...
            while not free:
                self.goal_pos = tuple(
                    self.np_random.choice(self.valid_pos_gen(margin=1))
                )
                next_goal = self._sanitize((self.goal_pos[0] + 1, self.goal_pos[1]))
                if self.grid.get(*next_goal) is None and not np.array_equal(
                    next_goal, self.agent_pos
                ):
                    free = True
            ...
            self.grid.set(
                self.goal_pos[0] + 1,
                self.goal_pos[1],
                LockBox(last_color, goal_lock=True, index=self.num_pairs),
            )

            if self.np_random.random() < self.unsolvable_prob:
                for path in range(self.num_paths):

                    if self.num_pairs > 1:
                        list_pairs = range(1, self.num_pairs)
                        idx = self.np_random.choice(list(list_pairs))
                        ...
                        for i in range(1, self.num_pairs):
                            ...
                            if i == idx:
                                ...
                                lock, key = key_lock
                                self.grid.set(
                                    *lock, LockBox(rand_color, index=-1, path=-1)
                                )
                                self.edges[path].remove(
                                    (i - 1 + multiplier, i + multiplier)
                                )

                            elif self.np_random.random() < self.edge_drop_prob:
                                key_lock = self.paths[path][i]
                                lock, key = key_lock
                                self.grid.set(
                                    *lock, LockBox(rand_color, index=-1, path=-1)
                        ...
                        self.grid.set(
                            self.goal_pos[0] + 1,
                            self.goal_pos[1],
                            LockBox(rand_color, index=-1, path=-1),
                        )
                        self.edges[path].remove(
                            (
                                self.num_pairs * (path + 1) - 1,
    ...
    def _gen_grid_once(self):
        last_color = None
        colors = (
            deepcopy(COLOR_NAMES) if not self.ood_colors else deepcopy(OOD_COLOR_NAMES)
        )
        for path in range(self.num_paths):
            ...
            attn_edges = []
            self.grid.set(
                *pairs[0], KeyBox(colors_path[0], index=0, first_key=True, path=path)
            )
            attn_edges.append((self.agent_pos, pairs[0]))
            for i, key_lock in enumerate(pairs[1:]):
                idx = i + 1
                lock, key = key_lock
                self.grid.set(*key, KeyBox(colors_path[idx], index=idx, path=path))
                self.grid.set(
                    *lock, LockBox(colors_path[idx - 1], index=idx, path=path)
                )
            ...
            edges.append(
                (self.num_pairs * (path + 1) - 1, self.num_pairs * self.num_paths)
            )
            self.paths[path] = pairs
            self.path_colors[path] = colors_path
            self.edges[path] = edges
            attn_edges.append(((pairs[-1][0], self.goal_pos)))
            self.attn_edges.append(attn_edges)
    ...
    def step(self, action):
        ...
        if action == self.actions.left:
            ...
            if (
                ...
                and self._current_key == self.num_pairs
                and self._current_path == fwd_cell.path
                and fwd_cell.unlockable
            ):
                terminated = True
                reward = self._reward()
        ...
        # pick up an object (this action is like interact for this env)
            ...
            if cur_cell is not None and cur_cell.can_pickup():
                if self._current_key is None and cur_cell.is_first_key():  # first key
                    ...
                    next_lock, _ = self.paths[cur_cell.path][
                        cur_cell.index + 1
                    ]  # we can open the next lock
                ...
                # opening a lock for which we have the key
                    ...
                    and cur_cell.path == self._current_path
                    and cur_cell.is_lock()
                ):
                    # first do the logic for if the lock is next to goal
                    self._current_key = cur_cell.index
                    self.unlocked_edges.append(
                        (cur_cell.index - 1, cur_cell.index)
                        if cur_cell.path == 0
                        else (
                            cur_cell.index - 1 + self.num_pairs,
                    ...
                    if cur_cell.is_goal_lock():
                        ...
                        lock_cell = self.grid.get(*next_lock)
                        assert (
                            lock_cell.is_lock() or lock_cell.is_goal()
                        ), "Not a lock or goal"
                        lock_cell.unlockable = True
    ...
    def _sample_key_pairs(self, num_pairs, colors_list):
        ...
        while not free:
            ...
            if (
                self.grid.get(*right_state) is None
                and self.grid.get(*left_state) is None
            ):
                free = True
        ...
        if state_right in valid_pos:
            valid_pos.remove(state_right)
        if state_left in valid_pos:
            valid_pos.remove(state_left)

        while len(pairs) < num_pairs:
            lock = tuple(self.np_random.choice(valid_pos))
            valid_pos.remove(self._sanitize(lock))
            if (
                lock[0] == 1
                or np.array_equal((lock[0] - 1, lock[1]), self.agent_pos)
                or self.grid.get(lock[0] - 1, lock[1]) is not None
            ):
                continue
            key = (lock[0] - 1, lock[1])
            key_sanitized = self._sanitize(key)
            if key_sanitized not in valid_pos:
                continue
            valid_pos.remove(key_sanitized)
            pairs.append((lock, key))

        colors = list(self.np_random.choice(colors_list, size=num_pairs, replace=False))
    ...
    def valid_pos_gen(self, margin=1):
        ...
        for x in range(self.width):
            for y in range(self.height):
                ...
                if obj is None:
                    if (
                        margin <= x <= self.width - margin - 1
                        and margin <= y <= self.height - margin - 1
                    ):
        ...
        for _, path in self.paths.items():
            ...
            if first_left in valid_pos_gen:
                valid_pos_gen.remove(first_left)
            if first_right in valid_pos_gen:
                valid_pos_gen.remove(first_right)
            ...
            for pair in range(1, len(path)):
                ...
                if left in valid_pos_gen:
                    valid_pos_gen.remove(left)
                if right in valid_pos_gen:
                    valid_pos_gen.remove(right)

        # goal state stuff
        if self.goal_pos:
            for i in range(1, 4):
                state = self._sanitize((self.goal_pos[0] + i, self.goal_pos[1]))
                if state in valid_pos_gen:
                    valid_pos_gen.remove(state)

        return valid_pos_gen

    def get_attn_edges(self):
        return deepcopy(self.attn_edges)
    ...
    def render_graph(self, figsize=(10, 6)):
        ...
        def custom_layout(
            G, num_pairs, n_paths, y_top=0.0, y_spacing=0.5, x_spacing=0.5
        ):
            pos = {}
        ...
        edge_colors = [
            "red" if e in self.unlocked_edges else "black" for e in G.edges()
        ]
...
class BoxWorldEnvV2(EnvBase):
    """Based off older BoxWorld env, this one instead has two keys and only one key leads
    to the goal.
    ...
    def __init__(
        ...
        self._current_context = 0
        self.num_distractors = num_distractors
        self.ratio_red_blue = ratio_red_blue
        self.num_pairs = num_pairs
        self._include_walls = include_walls
        self.goal_pos = None
        self.randomize_num_pairs = True if self.num_pairs == -1 else False
    ...
    def _gen_grid(self, width, height):
        ...
        if self._randomize:
            if self.randomize_num_pairs:
                self.num_pairs = self.np_random.integers(2, 5)
            # walls: for now simple just one wall
            if self._include_walls:
                self.walls = self._simple_wall_gen()
                for wall in self.walls:
    ...
    def _gen_grid_once(self):
        ...
        chosen_idx = 1 if self.np_random.random() < self.ratio_red_blue else 0
        self.chosen_keys = chosen_idx

        chosen_last_color = self.np_random.choice(colors)
        colors.remove(chosen_last_color)
        unchosen_last_color = self.np_random.choice(colors)
        colors.remove(unchosen_last_color)

        for path in range(self.num_paths):
            samples = self.num_pairs
            for color in ["green", "grey", "red", "darkcyan", "darkred"]:  # base objs
                if color in colors:
                    colors.remove(color)
            ...
            for key_idx, first_key in enumerate(["darkcyan", "darkred"]):
                pairs, colors_path, valid_pos = self._sample_key_pairs(
                    samples, colors, valid_pos
                )
                ...
                multiplier = path * (self.num_pairs)
                colors_path.insert(0, first_key)
                edges = []

                if key_idx == chosen_idx:
                    colors_path[-1] = chosen_last_color
                    attn_edges.append((self.agent_pos, pairs[0]))  #  maybe spurious?
                else:
                    colors_path[-1] = unchosen_last_color

                # add the first key here
                self.grid.set(
                    *pairs[0], KeyBox(first_key, index=0, first_key=True, path=path)
                )
                for i, key_lock in enumerate(pairs[1:]):
                    idx = i + 1
                    lock, key = key_lock
                    ...
                    edges.append((i + multiplier, idx + multiplier))
                    if chosen_idx == key_idx:
                        attn_edges.append((lock, key))

                if chosen_idx == key_idx:
                    pairs.append(
                        ((self.goal_pos[0] + 1, self.goal_pos[1]), self.goal_pos)
                    )
                    edges.append(
                        (
                            self.num_pairs * (path + 1) - 1,
                    ...
                    pairs.append(
                        ((self.goal_pos[0] + 1, self.goal_pos[1]), self.null_state)
                    )

                keys_paths[first_key] = pairs
                keys_edges[first_key] = edges
                keys_colors[first_key] = colors_path

            self.paths[path] = keys_paths
            self.path_colors[path] = keys_colors
            self.edges[path] = keys_edges
            attn_edges.append(
                (((self.goal_pos[0] + 1, self.goal_pos[1]), self.goal_pos))
            )
            self.attn_edges.append(attn_edges)
        ...
        self.grid.set(
            self.goal_pos[0] + 1,
            self.goal_pos[1],
            LockBox(chosen_last_color, goal_lock=True, index=self.num_pairs),
        )
    ...
    def step(self, action):  # TODO: update this for v2 and for v1
        self.step_count += 1
    ...
    def _sample_key_pairs(self, num_pairs, colors_list, valid_list):
        pairs = []
        valid_pos = valid_list.copy()
        free = False
        while not free:
            first_key = tuple(self.np_random.choice(valid_pos))
            right_state = self._sanitize((first_key[0] + 1, first_key[1]))
        ...
        valid_pos.remove(first_key)
        # remove states on left and right of first key
        state_right_right = self._sanitize((first_key[0] + 2, first_key[1]))
        state_left = self._sanitize((first_key[0] - 1, first_key[1]))
        if state_left in valid_pos:
            valid_pos.remove(state_left)
        if state_right_right in valid_pos:
            valid_pos.remove(state_right_right)
        ...
        while len(pairs) < num_pairs:
            ...
            if (
                ...
                or np.array_equal((lock[0] - 1, lock[1]), self.agent_pos)
                or self.grid.get(lock[0] - 1, lock[1]) is not None
                or self.grid.get(lock[0] - 2, lock[1]) is not None
            ):
                continue
            key = (lock[0] - 1, lock[1])
            key_sanitized = self._sanitize(key)
            # right_plus = self._sanitize((lock[0]+2, lock[1]))
            # left_of_key = self._sanitize((key[0]-1, key[1]))
            if key_sanitized not in valid_pos:
                continue
            # if right_plus not in valid_pos: continue
            # if left_of_key not in valid_pos: continue
            # valid_pos.remove(left_of_key)
            # valid_pos.remove(right_plus)
            valid_pos.remove(key_sanitized)
            pairs.append((lock, key))

        colors = list(
            self.np_random.choice(colors_list, size=num_pairs - 1, replace=False)
        )

        return pairs, colors, valid_pos
    ...
    def valid_pos_gen(self, margin=1):
        ...
        # check other path key-locks
        for _, path_combo in self.paths.items():
            for _, path in path_combo.items():
                if len(path) == 0:
                    continue
                first = path[0]
                first_left = self._sanitize((first[0] - 1, first[1]))
                first_right = self._sanitize((first[0] + 1, first[1]))
                first_right_right = self._sanitize((first[0] + 2, first[1]))
                if first_left in valid_pos_gen:
                    valid_pos_gen.remove(first_left)
                if first_right in valid_pos_gen:
                    valid_pos_gen.remove(first_right)
                if first_right_right in valid_pos_gen:
                    valid_pos_gen.remove(first_right_right)
