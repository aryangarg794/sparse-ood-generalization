...
from sparse_generalization.data.box_world.objects import Wall, Goal, KeyBox, LockBox, Ball, Box
...
class EnvBase(MiniGridEnv):
    ...
    def _simple_wall_gen(self, min_size=2):
        ...
        place = self.np_random.choice(list(range(2+min_size, self.size-2-min_size))) # at least 1 space at start, or 2 atr end
...
class BoxWorldEnv(EnvBase):
    ...
    def _gen_grid(self, width, height):
        ...
        if self._randomize:
            if self.randomize_num_pairs: self.num_pairs = self.np_random.integers(1, 5)
...
class BoxWorldEnvV2(EnvBase):
    ...
    def _gen_grid(self, width, height):
        ...
        if self._randomize:
            if self.randomize_num_pairs: self.num_pairs = self.np_random.integers(2, 5)
            # walls: for now simple just one wall
            if self._include_walls:
                self.walls = self._simple_wall_gen()
                for wall in self.walls:
    ...
    def _gen_grid_once(self):
        ...
        for path in range(self.num_paths):
            ...
            for key_idx, first_key in enumerate(['darkcyan', 'darkred']):
                pairs, colors_path, valid_pos = self._sample_key_pairs(samples, colors, valid_pos)
                ...
                self.grid.set(*pairs[0], KeyBox(first_key, index=0, first_key=True, path=path))
                for i, key_lock in enumerate(pairs[1:]):
                    idx = i+1
                    lock, key = key_lock
                    self.grid.set(*key, KeyBox(colors_path[idx], index=idx, path=path))
                ...
                if chosen_idx == key_idx:    
                    ...
                    pairs.append(((self.goal_pos[0]+1, self.goal_pos[1]), self.null_state))
            ...
            attn_edges.append((((self.goal_pos[0]+1, self.goal_pos[1]), self.goal_pos)))
            self.attn_edges.append(attn_edges)
        
        self.grid.set(self.goal_pos[0]+1, self.goal_pos[1], LockBox(chosen_last_color, goal_lock=True, index=self.num_pairs))
    ...
    def _sample_key_pairs(self, num_pairs, colors_list, valid_list):
        ...
        state_left = self._sanitize((first_key[0]-1, first_key[1]))
        if state_left in valid_pos: valid_pos.remove(state_left)
        if state_right_right in valid_pos: valid_pos.remove(state_right_right)
        
        while len(pairs) < num_pairs:
            lock = tuple(self.np_random.choice(valid_pos))
            valid_pos.remove(self._sanitize(lock))
            if lock[0] == 1 or np.array_equal((lock[0]-1, lock[1]), self.agent_pos) \
                or self.grid.get(lock[0]-1, lock[1]) is not None or \
                    self.grid.get(lock[0]-2, lock[1]) is not None: 
                continue
            key = (lock[0]-1, lock[1])
            key_sanitized = self._sanitize(key)
        ...
        colors = list(self.np_random.choice(colors_list, size=num_pairs-1, replace=False))
    ...
    def valid_pos_gen(self, margin=1):
        ...
        for _, path_combo in self.paths.items():
            for _, path in path_combo.items():
                ...
                if first_left in valid_pos_gen: valid_pos_gen.remove(first_left)
                if first_right in valid_pos_gen: valid_pos_gen.remove(first_right)
                if first_right_right in valid_pos_gen: valid_pos_gen.remove(first_right_right)
